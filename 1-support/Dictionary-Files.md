This page has been generated by `misc-hunspell/1-support/3-report.sh`. Do not edit this page manually. See also many other markdown documentation.

## Packages

The script `misc-hunspell/1-support/1-download.sh` will download the latest stable Hunspell and MySpell language support packages. Langauge support typically consists of an affix file and a dictionary file. Symbolic links are used to support some geografic regions where the particular language support also applies.

Some available packages are omitted from this overview and testing framework. Package `hunspell-fr` is only a dependency package. Packages `hunspell-fr-classical`, `hunspell-fr-modern` and `hunspell-fr-revised` conflict with `hunspell-fr-comprehensive`, which has a bigger affix file. Package `hunspell-gl` conflicts with `hunspell-gl-es`, which has a bigger affix file.

## Languages

A total of 90 different languages are available for Hunspell.

| Package | Version | Filename | Language |
|---|---|---|---|
| `hunspell-af` | `1%3a6.0.3-3` | `af_ZA` | Afrikaans |
| `hunspell-an` | `0.2-2` | `an_ES` | Aragonese |
| `hunspell-ar` | `3.2-1` | `ar` | Arabic |
| `hunspell-be` | `0.53-3` | `be_BY` | Belarusian |
| `hunspell-bg` | `1%3a6.0.3-3` | `bg_BG` | Bulgarian |
| `hunspell-bn` | `1%3a6.0.3-3` | `bn_BD` | Bengali |
| `hunspell-bo` | `0.4.0-1` | `bo` | Classical Tibetan |
| `hunspell-br` | `0.12-2` | `br_FR` | Breton |
| `hunspell-bs` | `1%3a6.0.3-3` | `bs_BA` | Bosnian |
| `hunspell-ca` | `3.0.2+repack1-2` | `ca` | Catalan |
| `hunspell-ca` | `3.0.2+repack1-2` | `ca_ES-valencia` | Valencian |
| `hunspell-cs` | `1%3a6.0.3-3` | `cs_CZ` | Czech |
| `hunspell-da` | `1%3a6.0.3-3` | `da_DK` | Danish |
| `hunspell-de-at` | `20161207-4` | `de_AT` | Austrian German |
| `hunspell-de-at-frami` | `1%3a6.0.3-3` | `de_AT_frami` | Austrian German |
| `hunspell-de-ch` | `20161207-4` | `de_CH` | Swiss German |
| `hunspell-de-ch-frami` | `1%3a6.0.3-3` | `de_CH_frami` | Swiss German |
| `hunspell-de-de` | `20161207-4` | `de_DE` | German |
| `hunspell-de-de-frami` | `1%3a6.0.3-3` | `de_DE_frami` | German |
| `hunspell-dz` | `0.1.0-1` | `dz` | Dzongkha |
| `hunspell-el` | `1%3a6.0.3-3` | `el_GR` | Modern Greek |
| `hunspell-en-au` | `1%3a2017.08.24` | `en_AU` | Australian English |
| `hunspell-en-ca` | `1%3a2017.08.24` | `en_CA` | Canadian English |
| `hunspell-en-gb` | `1%3a6.0.3-3` | `en_GB` | British English |
| `hunspell-en-us` | `1%3a2017.08.24` | `en_US` | American English |
| `hunspell-en-za` | `1%3a6.0.3-3` | `en_ZA` | South African English |
| `hunspell-es` | `1%3a6.0.3-3` | `es_ES` | Spanish |
| `hunspell-eu` | `0.5.20151110-2` | `eu` | Basque |
| `hunspell-fr-comprehensive` | `1%3a6.2-1` | `fr` | French |
| `hunspell-fy` | `2014` | `fy_NL` | ERROR: Unsupported language code fy_NL |
| `hunspell-gd` | `1%3a6.0.3-3` | `gd_GB` | Scottish Gaelic |
| `hunspell-gl-es` | `13.10-1` | `gl_ES` | Galician |
| `hunspell-gu` | `1%3a6.0.3-3` | `gu_IN` | Gujarati |
| `hunspell-gug` | `1%3a6.0.3-3` | `gug_PY` | Guarani |
| `hunspell-he` | `1%3a6.0.3-3` | `he_IL` | Hebrew |
| `hunspell-hi` | `1%3a6.0.3-3` | `hi_IN` | Hindi |
| `hunspell-hr` | `1%3a6.0.3-3` | `hr_HR` | Croatian |
| `hunspell-hu` | `1%3a6.0.3-3` | `hu_HU` | Hungarian |
| `hunspell-is` | `1%3a6.0.3-3` | `is_IS` | Icelandic |
| `hunspell-it` | `1%3a6.0.3-3` | `it_IT` | Italian |
| `hunspell-kk` | `1.1-2` | `kk_KZ` | Kazakh |
| `hunspell-kmr` | `1%3a6.0.3-3` | `kmr_Latn` | Kurmanji (Latin script) |
| `hunspell-ko` | `0.7.1-1` | `ko` | Korean |
| `hunspell-lo` | `1%3a6.0.3-3` | `lo_LA` | Laotian |
| `hunspell-lt` | `1%3a6.0.3-3` | `lt_LT` | Lithuanian |
| `hunspell-ml` | `0.1-2` | `ml_IN` | Malayalam |
| `hunspell-ne` | `1%3a6.0.3-3` | `ne_NP` | Nepalese |
| `hunspell-nl` | `2%3a2.10-6` | `nl` | Dutch |
| `hunspell-no` | `1%3a6.0.3-3` | `nb_NO` | Bokmål |
| `hunspell-no` | `1%3a6.0.3-3` | `nn_NO` | Nynorsk |
| `hunspell-oc` | `1%3a6.0.3-3` | `oc_FR` | Occitan |
| `hunspell-pl` | `1%3a6.0.3-3` | `pl_PL` | Polish |
| `hunspell-pt-br` | `1%3a6.0.3-3` | `pt_BR` | Brazilian |
| `hunspell-pt-pt` | `1%3a6.0.3-3` | `pt_PT` | Portuguese |
| `hunspell-ro` | `1%3a6.0.3-3` | `ro_RO` | Romanian |
| `hunspell-ru` | `1%3a6.0.3-3` | `ru_RU` | Russian |
| `hunspell-se` | `1.0~beta6.20081222-1.2` | `se` | North Sámi |
| `hunspell-si` | `1%3a6.0.3-3` | `si_LK` | Sinhala |
| `hunspell-sk` | `1%3a6.0.3-3` | `sk_SK` | Slovak |
| `hunspell-sl` | `1%3a6.0.3-3` | `sl_SI` | Slovene |
| `hunspell-sr` | `1%3a6.0.3-3` | `sr_Latn_RS` | Serbian (Latin script) |
| `hunspell-sr` | `1%3a6.0.3-3` | `sr_RS` | Serbian (Cyrillic script) |
| `hunspell-sv` | `1%3a6.0.3-3` | `sv_FI` | Finland Swedish |
| `hunspell-sv` | `1%3a6.0.3-3` | `sv_SE` | Swedish |
| `hunspell-sw` | `1%3a6.0.3-3` | `sw_TZ` | Swahili |
| `hunspell-te` | `1%3a6.0.3-3` | `te_IN` | Telugu |
| `hunspell-th` | `1%3a6.0.3-3` | `th_TH` | Thai |
| `hunspell-tlh` | `1.0.0-1` | `tlh_Latn` | Klingon (Latin script) |
| `hunspell-uk` | `1%3a6.0.3-3` | `uk_UA` | Ukrainian |
| `hunspell-uz` | `0.6-4` | `uz_UZ` | Uzbek |
| `hunspell-vi` | `1%3a6.0.3-3` | `vi_VN` | Vietnamese |
| `myspell-eo` | `2.1.2000.02.25-55` | `eo` | Esperanto |
| `myspell-et` | `1%3a20030606-27` | `et_EE` | Estonian |
| `myspell-fa` | `0.20070816-3` | `fa_IR` | Farsi |
| `myspell-fo` | `0.4.2-11` | `fo` | Faroese |
| `myspell-ga` | `2.0-25` | `ga_IE` | Irish |
| `myspell-gv` | `0.50-13` | `gv_GB` | Manx Gaelic |
| `myspell-hy` | `0.20.0-2` | `hy_AM` | Armenian |
| `myspell-lv` | `0.9.6-5` | `lv_LV` | Latvian |
| `myspell-nr` | `20070206-4ubuntu3` | `nr_ZA` | Ndebele |
| `myspell-ns` | `20070206-4ubuntu3` | `ns_ZA` | Northern Sotho |
| `myspell-sq` | `1.6.4-1` | `sq_AL` | Albanian |
| `myspell-ss` | `20070206-4ubuntu3` | `ss_ZA` | Swazi |
| `myspell-st` | `20070206-5ubuntu2` | `st_ZA` | Southern Sotho |
| `myspell-tl` | `0.4-0-17` | `tl` | Tagalog |
| `myspell-tn` | `20070206-4ubuntu3` | `tn_ZA` | Tswana |
| `myspell-ts` | `20070207-4ubuntu3` | `ts_ZA` | Tsonga |
| `myspell-ve` | `20070206-3ubuntu2` | `ve_ZA` | Venda |
| `myspell-xh` | `20070206-4ubuntu3` | `xh_ZA` | Xhosa |
| `myspell-zu` | `20070207-5ubuntu2` | `zu_ZA` | Zulu |

## Affix files

A total of 90 different affix files are available for Hunspell. Affix files which are made available via symbolic links are excluded. Note that each affix file has a unique name. Normally, these are installed in `/usr/share/hunspell/`.

| Package | Version | Filename | File Type | Intended Encoding | Lines |
|---|---|---|---|---|--:|
| `hunspell-af` | `1%3a6.0.3-3` | `af_ZA` | ISO-8859 | ISO8859-1 | `209` |
| `hunspell-an` | `0.2-2` | `an_ES` | ISO-8859 | ISO8859-1 | `1248` |
| `hunspell-ar` | `3.2-1` | `ar` | UTF-8 | UTF-8 | `26907` |
| `hunspell-be` | `0.53-3` | `be_BY` | UTF-8 | UTF-8 | `957` |
| `hunspell-bg` | `1%3a6.0.3-3` | `bg_BG` | ISO-8859 | microsoft-cp1251 | `1709` |
| `hunspell-bn` | `1%3a6.0.3-3` | `bn_BD` | UTF-8 | UTF-8 | `2` |
| `hunspell-bo` | `0.4.0-1` | `bo` | UTF-8 | UTF-8 | `73` |
| `hunspell-br` | `0.12-2` | `br_FR` | UTF-8 | UTF-8 | `433` |
| `hunspell-bs` | `1%3a6.0.3-3` | `bs_BA` | ISO-8859 | ISO8859-2 | `1028` |
| `hunspell-ca` | `3.0.2+repack1-2` | `ca` | UTF-8, with CRLF line terminators | UTF-8 | `13931` |
| `hunspell-ca` | `3.0.2+repack1-2` | `ca_ES-valencia` | UTF-8, with CRLF line terminators | UTF-8 | `13981` |
| `hunspell-cs` | `1%3a6.0.3-3` | `cs_CZ` | ISO-8859 | ISO8859-2 | `2597` |
| `hunspell-da` | `1%3a6.0.3-3` | `da_DK` | UTF-8, with very long lines | UTF-8 | `1529` |
| `hunspell-de-at` | `20161207-4` | `de_AT` | ISO-8859 | ISO8859-1 | `729` |
| `hunspell-de-at-frami` | `1%3a6.0.3-3` | `de_AT_frami` | ISO-8859 | ISO8859-1 | `729` |
| `hunspell-de-ch` | `20161207-4` | `de_CH` | ISO-8859 | ISO8859-1 | `729` |
| `hunspell-de-ch-frami` | `1%3a6.0.3-3` | `de_CH_frami` | ISO-8859 | ISO8859-1 | `729` |
| `hunspell-de-de` | `20161207-4` | `de_DE` | ISO-8859 | ISO8859-1 | `729` |
| `hunspell-de-de-frami` | `1%3a6.0.3-3` | `de_DE_frami` | ISO-8859 | ISO8859-1 | `729` |
| `hunspell-dz` | `0.1.0-1` | `dz` | UTF-8 | UTF-8 | `183` |
| `hunspell-el` | `1%3a6.0.3-3` | `el_GR` | ISO-8859 | ISO8859-7 | `820` |
| `hunspell-en-au` | `1%3a2017.08.24` | `en_AU` | UTF-8 | UTF-8 | `205` |
| `hunspell-en-ca` | `1%3a2017.08.24` | `en_CA` | UTF-8 | UTF-8 | `205` |
| `hunspell-en-gb` | `1%3a6.0.3-3` | `en_GB` | UTF-8 | UTF-8 | `1180` |
| `hunspell-en-us` | `1%3a2017.08.24` | `en_US` | UTF-8 | UTF-8 | `205` |
| `hunspell-en-za` | `1%3a6.0.3-3` | `en_ZA` | UTF-8 | UTF-8 | `1441` |
| `hunspell-es` | `1%3a6.0.3-3` | `es_ES` | UTF-8 | UTF-8 | `6891` |
| `hunspell-eu` | `0.5.20151110-2` | `eu` | ISO-8859 | ISO-8859-1 | `94543` |
| `hunspell-fr-comprehensive` | `1%3a6.2-1` | `fr` | UTF-8 | UTF-8 | `10918` |
| `hunspell-fy` | `2014` | `fy_NL` | ISO-8859 |  | `233221` |
| `hunspell-gd` | `1%3a6.0.3-3` | `gd_GB` | UTF-8 | UTF-8 | `374` |
| `hunspell-gl-es` | `13.10-1` | `gl_ES` | UTF-8 | UTF-8 | `12353` |
| `hunspell-gu` | `1%3a6.0.3-3` | `gu_IN` | UTF-8 | UTF-8 | `2` |
| `hunspell-gug` | `1%3a6.0.3-3` | `gug_PY` | UTF-8 | UTF-8 | `11` |
| `hunspell-he` | `1%3a6.0.3-3` | `he_IL` | UTF-8 | UTF-8 | `3370` |
| `hunspell-hi` | `1%3a6.0.3-3` | `hi_IN` | UTF-8 | UTF-8 | `2` |
| `hunspell-hr` | `1%3a6.0.3-3` | `hr_HR` | UTF-8 | UTF-8 | `3644` |
| `hunspell-hu` | `1%3a6.0.3-3` | `hu_HU` | Non-ISO extended-ASCII, with very long lines, with LF, NEL line terminators | UTF-8 | `50566` |
| `hunspell-is` | `1%3a6.0.3-3` | `is_IS` | UTF-8 | UTF-8 | `14739` |
| `hunspell-it` | `1%3a6.0.3-3` | `it_IT` | ISO-8859 | ISO8859-15 | `3407` |
| `hunspell-kk` | `1.1-2` | `kk_KZ` | UTF-8 (with BOM), with CRLF line terminators | UTF-8 | `2646` |
| `hunspell-kmr` | `1%3a6.0.3-3` | `kmr_Latn` | UTF-8 | UTF-8 | `234` |
| `hunspell-ko` | `0.7.1-1` | `ko` | UTF-8, with very long lines | UTF-8 | `122134` |
| `hunspell-lo` | `1%3a6.0.3-3` | `lo_LA` | ASCII | UTF-8 | `1` |
| `hunspell-lt` | `1%3a6.0.3-3` | `lt_LT` | ISO-8859 | ISO8859-13 | `2723` |
| `hunspell-ml` | `0.1-2` | `ml_IN` | ASCII | UTF-8 | `1` |
| `hunspell-ne` | `1%3a6.0.3-3` | `ne_NP` | UTF-8 | UTF-8 | `574` |
| `hunspell-nl` | `2%3a2.10-6` | `nl` | UTF-8 | UTF-8 | `1438` |
| `hunspell-no` | `1%3a6.0.3-3` | `nb_NO` | ISO-8859 | ISO8859-1 | `636` |
| `hunspell-no` | `1%3a6.0.3-3` | `nn_NO` | ISO-8859 | ISO8859-1 | `511` |
| `hunspell-oc` | `1%3a6.0.3-3` | `oc_FR` | ISO-8859 | ISO8859-15 | `763` |
| `hunspell-pl` | `1%3a6.0.3-3` | `pl_PL` | ISO-8859 | ISO8859-2 | `7286` |
| `hunspell-pt-br` | `1%3a6.0.3-3` | `pt_BR` | ISO-8859, with CRLF line terminators | ISO8859-1 | `27371` |
| `hunspell-pt-pt` | `1%3a6.0.3-3` | `pt_PT` | UTF-8 | UTF-8 | `1436` |
| `hunspell-ro` | `1%3a6.0.3-3` | `ro_RO` | UTF-8 | UTF-8 | `1847` |
| `hunspell-ru` | `1%3a6.0.3-3` | `ru_RU` | ISO-8859 | KOI8-R | `1633` |
| `hunspell-se` | `1.0~beta6.20081222-1.2` | `se` | UTF-8, with very long lines | UTF-8 | `376018` |
| `hunspell-si` | `1%3a6.0.3-3` | `si_LK` | UTF-8 | UTF-8 | `10032` |
| `hunspell-sk` | `1%3a6.0.3-3` | `sk_SK` | UTF-8 | UTF-8 | `2539` |
| `hunspell-sl` | `1%3a6.0.3-3` | `sl_SI` | ISO-8859 | ISO8859-2 | `565` |
| `hunspell-sr` | `1%3a6.0.3-3` | `sr_Latn_RS` | UTF-8 | UTF-8 | `20` |
| `hunspell-sr` | `1%3a6.0.3-3` | `sr_RS` | UTF-8 | UTF-8 | `28` |
| `hunspell-sv` | `1%3a6.0.3-3` | `sv_FI` | UTF-8 | UTF-8 | `856` |
| `hunspell-sv` | `1%3a6.0.3-3` | `sv_SE` | UTF-8 | UTF-8 | `856` |
| `hunspell-sw` | `1%3a6.0.3-3` | `sw_TZ` | ASCII | ISO8859-1 | `27` |
| `hunspell-te` | `1%3a6.0.3-3` | `te_IN` | UTF-8 | UTF-8 | `2` |
| `hunspell-th` | `1%3a6.0.3-3` | `th_TH` | ISO-8859 | TIS-620 | `7` |
| `hunspell-tlh` | `1.0.0-1` | `tlh_Latn` | UTF-8 | UTF-8 | `108` |
| `hunspell-uk` | `1%3a6.0.3-3` | `uk_UA` | UTF-8 | UTF-8 | `4495` |
| `hunspell-uz` | `0.6-4` | `uz_UZ` | ASCII | UTF-8 | `1` |
| `hunspell-vi` | `1%3a6.0.3-3` | `vi_VN` | UTF-8 | UTF-8 | `42` |
| `myspell-eo` | `2.1.2000.02.25-55` | `eo` | ISO-8859 | ISO8859-3 | `1449` |
| `myspell-et` | `1%3a20030606-27` | `et_EE` | ISO-8859 | ISO8859-15 | `9330` |
| `myspell-fa` | `0.20070816-3` | `fa_IR` | ASCII | UTF-8 | `1` |
| `myspell-fo` | `0.4.2-11` | `fo` | ISO-8859 | ISO8859-1 | `663` |
| `myspell-ga` | `2.0-25` | `ga_IE` | ISO-8859 | ISO8859-1 | `611` |
| `myspell-gv` | `0.50-13` | `gv_GB` | ASCII | ISO8859-1 | `6` |
| `myspell-hy` | `0.20.0-2` | `hy_AM` | UTF-8 | UTF-8 | `1008` |
| `myspell-lv` | `0.9.6-5` | `lv_LV` | ISO-8859 | ISO8859-13 | `2689` |
| `myspell-nr` | `20070206-4ubuntu3` | `nr_ZA` | ASCII | ISO8859-1 | `22` |
| `myspell-ns` | `20070206-4ubuntu3` | `ns_ZA` | ISO-8859 | ISO8859-15 | `21` |
| `myspell-sq` | `1.6.4-1` | `sq_AL` | ISO-8859 | ISO8859-1 | `298` |
| `myspell-ss` | `20070206-4ubuntu3` | `ss_ZA` | ISO-8859 | ISO8859-1 | `21` |
| `myspell-st` | `20070206-5ubuntu2` | `st_ZA` | ISO-8859 | ISO8859-1 | `21` |
| `myspell-tl` | `0.4-0-17` | `tl` | ISO-8859 | ISO8859-1 | `88` |
| `myspell-tn` | `20070206-4ubuntu3` | `tn_ZA` | ISO-8859 | ISO8859-2 | `21` |
| `myspell-ts` | `20070207-4ubuntu3` | `ts_ZA` | ASCII | ISO8859-1 | `21` |
| `myspell-ve` | `20070206-3ubuntu2` | `ve_ZA` | ISO-8859 | UTF-8 | `21` |
| `myspell-xh` | `20070206-4ubuntu3` | `xh_ZA` | ASCII | ISO8859-1 | `21` |
| `myspell-zu` | `20070207-5ubuntu2` | `zu_ZA` | ASCII | ISO8859-1 | `21212` |

## Dictionary files

A total of 92 different dictionary files are available for Hunspell. Dictionary files which are made available via symbolic links are excluded. Note that each dictionary file has a unique name. Normally, these are installed in `/usr/share/hunspell/`. Note that medical extention dictionary files, see `-med`, `_med` and `_MED`, do not have their own affix file. These dictionaries can be loaded additionally.

| Package | Version | Filename | File Type | Lines |
|---|---|---|---|--:|
| `hunspell-af` | `1%3a6.0.3-3` | `af_ZA.dic` | ISO-8859 | `125473` |
| `hunspell-an` | `0.2-2` | `an_ES.dic` | data | `20862` |
| `hunspell-ar` | `3.2-1` | `ar.dic` | UTF-8 | `170813` |
| `hunspell-be` | `0.53-3` | `be_BY.dic` | UTF-8 | `82080` |
| `hunspell-bg` | `1%3a6.0.3-3` | `bg_BG.dic` | ISO-8859 | `78239` |
| `hunspell-bn` | `1%3a6.0.3-3` | `bn_BD.dic` | UTF-8 | `110751` |
| `hunspell-bo` | `0.4.0-1` | `bo.dic` | UTF-8 | `379` |
| `hunspell-br` | `0.12-2` | `br_FR.dic` | UTF-8 | `465038` |
| `hunspell-bs` | `1%3a6.0.3-3` | `bs_BA.dic` | ISO-8859 | `30543` |
| `hunspell-ca` | `3.0.2+repack1-2` | `ca.dic` | UTF-8, with CRLF line terminators | `133381` |
| `hunspell-ca` | `3.0.2+repack1-2` | `ca_ES-valencia.dic` | UTF-8, with CRLF line terminators | `133493` |
| `hunspell-cs` | `1%3a6.0.3-3` | `cs_CZ.dic` | ISO-8859 | `166566` |
| `hunspell-da` | `1%3a6.0.3-3` | `da_DK.dic` | UTF-8 | `161316` |
| `hunspell-de-at` | `20161207-4` | `de_AT.dic` | ISO-8859 | `75874` |
| `hunspell-de-at-frami` | `1%3a6.0.3-3` | `de_AT_frami.dic` | ISO-8859 | `258243` |
| `hunspell-de-ch` | `20161207-4` | `de_CH.dic` | ISO-8859 | `75704` |
| `hunspell-de-ch-frami` | `1%3a6.0.3-3` | `de_CH_frami.dic` | ISO-8859 | `257827` |
| `hunspell-de-de` | `20161207-4` | `de_DE.dic` | ISO-8859 | `75608` |
| `hunspell-de-de-frami` | `1%3a6.0.3-3` | `de_DE_frami.dic` | ISO-8859 | `258218` |
| `hunspell-de-med` | `20160103-1` | `de_med.dic` | ISO-8859 | `9972` |
| `hunspell-dz` | `0.1.0-1` | `dz.dic` | UTF-8 | `405` |
| `hunspell-el` | `1%3a6.0.3-3` | `el_GR.dic` | ISO-8859 | `828807` |
| `hunspell-en-au` | `1%3a2017.08.24` | `en_AU.dic` | ASCII | `49427` |
| `hunspell-en-ca` | `1%3a2017.08.24` | `en_CA.dic` | ASCII | `49235` |
| `hunspell-en-gb` | `1%3a6.0.3-3` | `en_GB.dic` | UTF-8 | `65867` |
| `hunspell-en-med` | `0.0.20080513-2` | `en_MED.dic` | ISO-8859, with CRLF, LF line terminators | `48338` |
| `hunspell-en-us` | `1%3a2017.08.24` | `en_US.dic` | ASCII | `49058` |
| `hunspell-en-za` | `1%3a6.0.3-3` | `en_ZA.dic` | UTF-8 | `53542` |
| `hunspell-es` | `1%3a6.0.3-3` | `es_ES.dic` | UTF-8 | `58947` |
| `hunspell-eu` | `0.5.20151110-2` | `eu.dic` | ISO-8859 | `144690` |
| `hunspell-fr-comprehensive` | `1%3a6.2-1` | `fr.dic` | UTF-8 | `82599` |
| `hunspell-fy` | `2014` | `fy_NL.dic` | ISO-8859 | `299504` |
| `hunspell-gd` | `1%3a6.0.3-3` | `gd_GB.dic` | UTF-8 | `331834` |
| `hunspell-gl-es` | `13.10-1` | `gl_ES.dic` | UTF-8 | `53933` |
| `hunspell-gu` | `1%3a6.0.3-3` | `gu_IN.dic` | UTF-8 | `168957` |
| `hunspell-gug` | `1%3a6.0.3-3` | `gug_PY.dic` | UTF-8 | `4217` |
| `hunspell-he` | `1%3a6.0.3-3` | `he_IL.dic` | UTF-8 | `469751` |
| `hunspell-hi` | `1%3a6.0.3-3` | `hi_IN.dic` | UTF-8 | `15991` |
| `hunspell-hr` | `1%3a6.0.3-3` | `hr_HR.dic` | UTF-8 | `53518` |
| `hunspell-hu` | `1%3a6.0.3-3` | `hu_HU.dic` | UTF-8 | `93814` |
| `hunspell-is` | `1%3a6.0.3-3` | `is_IS.dic` | UTF-8 | `193171` |
| `hunspell-it` | `1%3a6.0.3-3` | `it_IT.dic` | ISO-8859 | `95186` |
| `hunspell-kk` | `1.1-2` | `kk_KZ.dic` | UTF-8 (with BOM), with CRLF line terminators | `54063` |
| `hunspell-kmr` | `1%3a6.0.3-3` | `kmr_Latn.dic` | UTF-8 | `4760` |
| `hunspell-ko` | `0.7.1-1` | `ko.dic` | UTF-8 | `102252` |
| `hunspell-lo` | `1%3a6.0.3-3` | `lo_LA.dic` | UTF-8 | `11207` |
| `hunspell-lt` | `1%3a6.0.3-3` | `lt_LT.dic` | ISO-8859 | `83259` |
| `hunspell-ml` | `0.1-2` | `ml_IN.dic` | UTF-8 | `142592` |
| `hunspell-ne` | `1%3a6.0.3-3` | `ne_NP.dic` | UTF-8, with CRLF, LF line terminators | `39925` |
| `hunspell-nl` | `2%3a2.10-6` | `nl.dic` | UTF-8 | `173579` |
| `hunspell-no` | `1%3a6.0.3-3` | `nb_NO.dic` | ISO-8859 | `254011` |
| `hunspell-no` | `1%3a6.0.3-3` | `nn_NO.dic` | ISO-8859 | `214830` |
| `hunspell-oc` | `1%3a6.0.3-3` | `oc_FR.dic` | ISO-8859 | `56579` |
| `hunspell-pl` | `1%3a6.0.3-3` | `pl_PL.dic` | ISO-8859 | `308305` |
| `hunspell-pt-br` | `1%3a6.0.3-3` | `pt_BR.dic` | Non-ISO extended-ASCII, with CRLF line terminators | `312369` |
| `hunspell-pt-pt` | `1%3a6.0.3-3` | `pt_PT.dic` | UTF-8 | `44214` |
| `hunspell-ro` | `1%3a6.0.3-3` | `ro_RO.dic` | UTF-8 | `180888` |
| `hunspell-ru` | `1%3a6.0.3-3` | `ru_RU.dic` | ISO-8859, with CRLF, LF line terminators | `146270` |
| `hunspell-se` | `1.0~beta6.20081222-1.2` | `se.dic` | UTF-8 | `527512` |
| `hunspell-si` | `1%3a6.0.3-3` | `si_LK.dic` | UTF-8, with very long lines | `30320` |
| `hunspell-sk` | `1%3a6.0.3-3` | `sk_SK.dic` | UTF-8 | `247006` |
| `hunspell-sl` | `1%3a6.0.3-3` | `sl_SI.dic` | ISO-8859 | `246857` |
| `hunspell-sr` | `1%3a6.0.3-3` | `sr_Latn_RS.dic` | UTF-8 | `222289` |
| `hunspell-sr` | `1%3a6.0.3-3` | `sr_RS.dic` | UTF-8 | `222289` |
| `hunspell-sv` | `1%3a6.0.3-3` | `sv_FI.dic` | UTF-8 | `151839` |
| `hunspell-sv` | `1%3a6.0.3-3` | `sv_SE.dic` | UTF-8 | `152176` |
| `hunspell-sw` | `1%3a6.0.3-3` | `sw_TZ.dic` | ASCII | `67901` |
| `hunspell-te` | `1%3a6.0.3-3` | `te_IN.dic` | UTF-8 | `125084` |
| `hunspell-th` | `1%3a6.0.3-3` | `th_TH.dic` | ISO-8859 | `38870` |
| `hunspell-tlh` | `1.0.0-1` | `tlh_Latn.dic` | ASCII | `3645` |
| `hunspell-uk` | `1%3a6.0.3-3` | `uk_UA.dic` | UTF-8 | `111403` |
| `hunspell-uz` | `0.6-4` | `uz_UZ.dic` | UTF-8 | `97001` |
| `hunspell-vi` | `1%3a6.0.3-3` | `vi_VN.dic` | UTF-8 | `6632` |
| `myspell-eo` | `2.1.2000.02.25-55` | `eo.dic` | ISO-8859 | `18219` |
| `myspell-et` | `1%3a20030606-27` | `et_EE.dic` | ISO-8859 | `282174` |
| `myspell-fa` | `0.20070816-3` | `fa_IR.dic` | UTF-8 | `331789` |
| `myspell-fo` | `0.4.2-11` | `fo.dic` | ISO-8859 | `396161` |
| `myspell-ga` | `2.0-25` | `ga_IE.dic` | ISO-8859 | `65287` |
| `myspell-gv` | `0.50-13` | `gv_GB.dic` | ISO-8859 | `32359` |
| `myspell-hy` | `0.20.0-2` | `hy_AM.dic` | UTF-8 | `63808` |
| `myspell-lv` | `0.9.6-5` | `lv_LV.dic` | ISO-8859 | `156215` |
| `myspell-nr` | `20070206-4ubuntu3` | `nr_ZA.dic` | ASCII | `12747` |
| `myspell-ns` | `20070206-4ubuntu3` | `ns_ZA.dic` | ASCII | `4935` |
| `myspell-sq` | `1.6.4-1` | `sq_AL.dic` | ISO-8859 | `229506` |
| `myspell-ss` | `20070206-4ubuntu3` | `ss_ZA.dic` | ASCII | `18970` |
| `myspell-st` | `20070206-5ubuntu2` | `st_ZA.dic` | ASCII | `6457` |
| `myspell-tl` | `0.4-0-17` | `tl.dic` | ISO-8859 | `16428` |
| `myspell-tn` | `20070206-4ubuntu3` | `tn_ZA.dic` | ASCII | `10866` |
| `myspell-ts` | `20070207-4ubuntu3` | `ts_ZA.dic` | ASCII | `28355` |
| `myspell-ve` | `20070206-3ubuntu2` | `ve_ZA.dic` | UTF-8 | `8787` |
| `myspell-xh` | `20070206-4ubuntu3` | `xh_ZA.dic` | ASCII | `18122` |
| `myspell-zu` | `20070207-5ubuntu2` | `zu_ZA.dic` | ASCII | `73250` |

## File types

The following pairs of affix file and dictionary file have a different file type. This might be a problem in some cases.

| Language | Affix file type | Dictionary file type |
|---|---|---|
| `an_ES` | ISO-8859 text | data |
| `hu_HU` | Non-ISO extended-ASCII text, with LF, NEL line terminators | UTF-8 Unicode text |
| `ne_NP` | UTF-8 Unicode text | UTF-8 Unicode text, with CRLF, LF line terminators |
| `pt_BR` | ISO-8859 text, with CRLF line terminators | Non-ISO extended-ASCII text, with CRLF line terminators |
| `ru_RU` | ISO-8859 text | ISO-8859 text, with CRLF, LF line terminators |
| `gv_GB` | ASCII text | ISO-8859 text |
| `ns_ZA` | ISO-8859 text | ASCII text |
| `ss_ZA` | ISO-8859 text | ASCII text |
| `st_ZA` | ISO-8859 text | ASCII text |
| `tn_ZA` | ISO-8859 text | ASCII text |
| `ve_ZA` | ISO-8859 text | UTF-8 Unicode text |
