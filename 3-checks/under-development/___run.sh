...
do
    ...
    do
        ...
        do
            ...
            wordlist=`../0-tools/language_support_to_wordlist_name.sh $package $language`
            if [ `echo $wordlist|grep -c ERROR` = 0 ]; then
                ...
                do
                    ...
                    for wordlist_path in `find ../2-wordlists/packages/$wordlist_package/$wordlist_version/usr/share/dict/ -type f -name $wordlist_file`  # Use find for file here, not ls. This will break if a dict file is replaced by a symbolic link. That will show up in the results and require changing ../0-tools/language_support_to_wordlist_name.sh
                    do
